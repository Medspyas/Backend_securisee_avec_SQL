...
from utils import get_valid_email, get_valid_float, get_valid_integer, hash_password

"""
    Ce fichier regroupe les fonctionnalités accessibles au rôle 'gestion'.
    Il permet :
        - la création, modification et suppression des utilisateurs,
        - l'assignation des supports aux événements,
        - la création et modification de tous les contrats,
        - la consultation globale des clients, contrats et événements.
...
def manage_create_user():
    ...
    try:
        ...
        user = gestion_manager.create_user(
...
def manage_assign_support():
    ...
    try:
        ...
        for event in events:
            print(
                f"[{event.id}] {event.event_name} ({event.event_date_start}) - Client ID : {event.client_id}"
            )
